...
void ConvertESDTrack(TString esdFileName, TString outFileName = "AliESDs.in",
                     Bool_t refit = kFALSE, Int_t LastEvent = -1) {
  ...
  Int_t nevents = (LastEvent >= 0)
                      ? TMath::Min(LastEvent + 1, (Int_t)tree->GetEntries())
  ...
  for (Int_t iEvent = 0; iEvent < nevents; iEvent++) {
    ...
    if (iEvent == 0 && refit) {
      ...
      auto field = o2::field::MagneticField::createFieldMap(
          -30000., -5999.95, o2::field::MagneticField::kConvLHC, false, 3500.,
    ...
    for (Int_t iTrack = 0; iTrack < nTracks; iTrack++) {
      ...
      if (!track->ContainTrackerData())
        continue;
      ++nTrkTracks;
      size += sizeof(TrackParamStruct) + sizeof(Int_t) +
              track->GetNClusters() * sizeof(ClusterStruct);
    }
    if (nTrkTracks > 0)
      size += sizeof(Int_t);
    ...
    for (Int_t iTrack = 0; iTrack < nTracks; iTrack++) {
      ...
      AliMUONTrackParam *param = static_cast<AliMUONTrackParam *>(
          muonTrack.GetTrackParamAtCluster()->First());
      /*
      ...
      */
      ...
      for (Int_t iCl = 0; iCl < nClusters; iCl++) {
        ...
        AliESDMuonCluster *cluster =
            esd->FindMuonCluster(track->GetClusterId(iCl));
